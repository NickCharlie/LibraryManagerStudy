 ...
 * 输出级别:
 * debug: 输出 debug, info, warn, error </li>
 * info: 输出 info, warn, error </li>
 * warn: 输出 warn, error </li>
 * error: 输出 error </li>
...
public class LogUtils {
    ...
    public static void init() {
        // 设置日志输出级别, 默认为 INFO 级别
        LogUtils.setLogOutLevel(Level.WARN);
        // 设置日志输出文件(追加到文件尾部)
        try {
            LogUtils.setLogOutFile(new File("Logging.log"));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        // 设置日志输出位置(是否输出到控制台 和 是否输出到文件), 默认只输出到控制台, 不输出到文件
    ...
    public static void error(String tag, Exception e) {
        ...
        try {
            ...
            printOut.flush();
            error(tag, new String(bytesBufOut.toByteArray(), StandardCharsets.UTF_8));
